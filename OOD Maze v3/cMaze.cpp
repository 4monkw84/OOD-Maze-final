...
void    cMaze::reset()
{
	...
	m_msLimit = 0;
	m_start.x = m_start.y = a_exit.x = a_exit.y = m_dimensions.x = m_dimensions.y = 0;
}
...
void        cMaze::print(int startCol, int startRow)
{
	...
	for (auto s : m_strings)
	{
		...
		SetConsoleTextAttribute(hOut, 8); // set console text colour to white for maze walls
		std::cout << s;
		++position.Y;
	}
...
//-----------------------------------------------------------------------------
bool    cMaze::getRandomEmptyEndpoint(int& col, int& row, int nAttempts)
{
	if (0 == nAttempts)
		nAttempts = m_dimensions.x * m_dimensions.y;
	do
	{
		cVector2* endpoint = &m_endpoints.at(m_mtRand() % m_endpoints.size());

		// is position is empty
		if (p_chars[endpoint->y][endpoint->x] == m_floorChar)
		{
			col = endpoint->x;
			row = endpoint->y;
			return true;
